...
class websettings
{
    ...
    public function get_combo($obj = null, $params = null)
    {
        $views = null;
        $array = array();
        ...
        if ($obj) {
            foreach ($obj as $row) {
                switch ($params['type']) {
                    case 'select':
                        if (in_array($row->id, $params['selected'])) {
                            $views .= '<option value="' . $row->id . '"  selected>' . $row->description . '</option>';
                        } else {
                            $views .= '<option value="' . $row->id . '">' . $row->description . '</option>';
                        }
                        break;
                    case 'slinks':
                        $array[] = '<a href="' . $this->_link_home . '/' . $row->id . '">' . $row->description . '</a>';
                        break;
                    case 'llinks':
                        $views .= '<li><a href="' . $this->_link_home . '/' . $row->id . '">' . $row->description . '</a></li>';
                        break;
                    case 'header':
                        $views .= '<option value="' . $row->id . '">' . $row->description . '</option>';
                        break;
