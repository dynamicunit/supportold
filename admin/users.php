...
try {
    ...
    $columns = 'id, full_name, phone, email, registration_ip_address, profile_image, membership, is_active, is_email_verified, created_at';
    $table = 'users';
    $joins = [];
    $conditions = [
        'website_id' => ['=', 1]
    ...
    if ($user->fetchbyjson($columns, $table, $joins, $conditions, $groupBy, $orderBy, $limit, $offset)) {
        ...
        if (!empty($dataarray['data'])) {
            ...
            $totalPages = ceil($user->get_total_count() / $common->get_value('per_page'));
            ...
            // Get the pagination
            $render_pagination = $user->get_pagination(
    ...
    if ($e->getMessage() === 'Pages not found.') {
