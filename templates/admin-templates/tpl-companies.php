            ...
            <div class="row">
                <div class="col">
                    <div class="d-flex align-items-center justify-content-between">
                        <h1>Companies</h1>
                        <div>
                            <a href="<?= $baseurl ?>/admin/manage-company" class="btn btn-sm btn-outline-primary"><i
                            ...
                            <tbody>
                                <?php if ($render) {
                                    foreach ($render as $key => $row): ?>
                                        <tr id="company_<?= $row['id'] ?>">

                                            <td><?= $key + 1 ?></td>
                                            <td><img style="width: 50px; height: 50px;"
                                                    src="<?= htmlspecialchars($row['logo_url']) ?>"
                                                    alt="<?= htmlspecialchars($row['company_name']) ?>"></td>
                                            ...
                                            <td>
                                                <a class="btn btn-sm btn-outline-primary"
                                                    href="<?= $baseurl ?>/admin/manage-company/<?= $row['id'] ?>"><i
                                                        class="icon_pencil"></i></a>
                                                <a href="#" class="btn btn-sm btn-outline-primary delete-company"
                                                    data-company-id="<?= $row['id'] ?>"><i class="icon_trash"></i></a>
                                            </td>
                                        </tr>
                                    <?php endforeach;
                                } else {
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.delete-company').forEach(function (button) {
                button.addEventListener('click', function () {
                    deleteCompanyId = this.getAttribute('data-company-id');
            ...
            document.getElementById('confirmDeleteCompany').addEventListener('click', function () {
                ...
                formData.append('id', deleteCompanyId);
                formData.append('token', csrfToken);

                fetch('<?= $baseurl ?>/admin/control/delete-company.php', {
                    method: 'POST',
                    body: formData
                })
                    .then(response => {
                        fetchCsrfToken();
                        if (!response.ok) {
                            throw new Error('Error: Could not delete the company.');
                        }
                        return response.json();
                    })
                    .then(data => {
                        if (data.success) {
                            var postElement = document.getElementById('company_' + deleteCompanyId);
                            if (postElement) {
                                postElement.remove();
                            }

                            var deleteCompanyModal = bootstrap.Modal.getInstance(document.getElementById('deleteCompanyModal'));
