            ...
            <div class="row">
                <div class="col">
                    <div class="d-flex align-items-center justify-content-between">
                        <h1>Lookup Codes</h1>
                        <div>
                            <a href="<?= $baseurl ?>/admin/manage-lookup-code" class="btn btn-outline-primary btn-sm"><i
                                        ...
                                        <tr id="post_<?= $row['id'] ?>">
                                            ...
                                            <td>
                                                <a class="btn btn-sm btn-outline-primary"
                                                    href="<?= $baseurl ?>/admin/manage-lookup-code/<?= $row['id'] ?>"><i
                                                        class="icon_pencil"></i></a>
                                                <a href="#" class="btn btn-sm btn-outline-primary delete-post"
                                                    data-lookup-id="<?= $row['id'] ?>"><i class="icon_trash"></i></a>
                                            </td>
                                        </tr>
                                    <?php endforeach;
                                } else {
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="deletePostModalLabel">Delete Lookup</h5>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.delete-post').forEach(function (button) {
                button.addEventListener('click', function () {
                    deletePostId = this.getAttribute('data-lookup-id');
            ...
            document.getElementById('confirmDeletePost').addEventListener('click', function () {
                ...
                fetch('<?= $baseurl ?>/admin/control/delete-lookup.php', {
                    method: 'POST',
                    body: formData
                })
                    .then(response => {
