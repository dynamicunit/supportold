...
try {
    ...
    $columns = 'website_id, id, image, blog_title,meta_desc, updated_at';
    $table = 'blogs';
    $joins = [];
    $conditions = [
    ...
    if ($blog->fetchbyjson($columns, $table, $joins, $conditions, $groupBy, $orderBy, $limit, $offset)) {
        ...
        if (!empty($dataarray['data'])) {
            ...
            $totalPages = ceil($blog->get_total_count() / $common->get_value('per_page'));
            ...
            // Get the pagination
            $render_pagination = $blog->get_pagination(
